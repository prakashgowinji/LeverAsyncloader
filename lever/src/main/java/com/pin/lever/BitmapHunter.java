...
import static com.pin.lever.Lever.Priority.LOW;
import static com.pin.lever.Utils.OWNER_HUNTER;
import static com.pin.lever.Utils.VERB_DECODED;
import static com.pin.lever.Utils.VERB_EXECUTING;
import static com.pin.lever.Utils.VERB_JOINED;
import static com.pin.lever.Utils.VERB_REMOVED;
import static com.pin.lever.Utils.VERB_TRANSFORMED;
...
class BitmapHunter implements Runnable {
    ...
    Bitmap result;
    Future<?> future;
    Lever.LoadedFrom loadedFrom;
    Exception exception;
    int exifOrientation; // Determined during decoding of original resource.
    int retryCount;
    Priority priority;

    BitmapHunter(Lever lever, Dispatcher dispatcher, Cache cache, Stats stats, Action action,
    ...
    Lever.LoadedFrom getLoadedFrom() {
        return loadedFrom;
    }
    ...
    static BitmapHunter forRequest(Lever lever, Dispatcher dispatcher, Cache cache, Stats stats,
                                   Action action) {
        Request request = action.getRequest();
        List<RequestHandler> requestHandlers = lever.getRequestHandlers();
        ...
        for (int i = 0, count = requestHandlers.size(); i < count; i++) {
            ...
            if (requestHandler.canHandleRequest(request)) {
                return new BitmapHunter(lever, dispatcher, cache, stats, action, requestHandler);
            }
        }

        return new BitmapHunter(lever, dispatcher, cache, stats, action, ERRORING_HANDLER);
    }
