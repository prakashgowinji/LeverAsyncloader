...
import static com.pin.lever.MediaStoreRequestHandler.LeverKind.FULL;
import static com.pin.lever.MediaStoreRequestHandler.LeverKind.MICRO;
import static com.pin.lever.MediaStoreRequestHandler.LeverKind.MINI;
...
class MediaStoreRequestHandler extends ContentStreamRequestHandler {
    ...
    @Override public Result load(Request request, int networkPolicy) throws IOException {
        ...
        if (request.hasSize()) {
            LeverKind leverKind = getLeverKind(request.targetWidth, request.targetHeight);
            if (!isVideo && leverKind == FULL) {
            ...
            calculateInSampleSize(request.targetWidth, request.targetHeight, leverKind.width,
            ...
            if (isVideo) {
                ...
                int kind = (leverKind == FULL) ? MediaStore.Video.Thumbnails.MINI_KIND : leverKind.androidKind;
                bitmap = MediaStore.Video.Thumbnails.getThumbnail(contentResolver, id, kind, options);
            } else {
                bitmap =
                        MediaStore.Images.Thumbnails.getThumbnail(contentResolver, id, leverKind.androidKind, options);
            }
    ...
    static LeverKind getLeverKind(int targetWidth, int targetHeight) {
    ...
    enum LeverKind {
        ...
        LeverKind(int androidKind, int width, int height) {
